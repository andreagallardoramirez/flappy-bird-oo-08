...
En el lab anterior lograste eliminar los obstaculos que no se ven en la pantalla. En este lab vamos a agregar más código JavaScript para que cuando `bird` tenga una colisión con `obstaculos`, el `juego` termine.
...
1. Agrega el método `bird.colisionY`
2. Agrega el método `bird.colisionX`
3. Dentro del método `bird.colision`, itera sobre `obstaculos.lista`. Si `bird.colisionX()` y `bird.colisionY()` son verdad, llama a `juego.termina()` y agrega el id "colision" a topObstacle y bottomObstacle.
